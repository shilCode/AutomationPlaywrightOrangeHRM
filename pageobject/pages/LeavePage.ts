...
export class LeavePage {
  ...
  constructor(page: Page) {
    ...
    this.applyLeaveFullView = page.locator(
      '[class="orangehrm-card-container"]',
    );
    this.applyLeaveHeading = this.applyLeaveFullView.getByRole("heading", {
      name: "Apply Leave",
    });
    this.applyLeaveBtn = this.applyLeaveFullView.getByRole("button", {
      name: "Apply",
    });
    this.applyLeaveTypesStr = this.applyLeaveFullView
      .locator("div")
      .filter({ hasText: /^Leave Type$/ });
    this.applyLeaveTypesDropDown = this.applyLeaveFullView
      .locator("div")
      .filter({ hasText: /^-- Select --$/ })
      .nth(2);
    this.applyLeaveTypesDropDownOption = this.applyLeaveFullView.getByRole(
      "option",
      { name: "US - Personal" },
    );
    this.applyLeaveCalenderStartDate = this.applyLeaveFullView
      .getByPlaceholder("yyyy-dd-mm")
      .first();
    this.applyLeaveCalenderEndDate = this.applyLeaveFullView
      .getByPlaceholder("yyyy-dd-mm")
      .last();
    this.applyLeaveCalenderView = page.locator(
      '[class*="oxd-date-input-calendar"]',
    );
    this.applyLeaveCalenderViewTodayStr =
      this.applyLeaveCalenderView.getByText("Today");
    this.applyLeaveCalenderViewCloseStr =
      this.applyLeaveCalenderView.getByText("Close");
  ...
  async resetMyLeave() {
    await expect(
      this.page
        .locator('[class="oxd-table-filter"]')
        .getByRole("heading", { name: "My Leave List" }),
    ).toContainText("My Leave List");
    await this.page
      .locator('[class="oxd-table-filter"]')
      .getByRole("button", { name: "Reset" })
      .click();
    await expect(
      this.page.locator('[class="oxd-toast-content oxd-toast-content--info"]'),
    ).toContainText("");
    expect(
      this.page
        .locator('[class="orangehrm-paper-container"]')
        .getByText("No Records Found"),
    ).toMatch(/No Records Found/);
  }
}
